...
// สร้าง websockets server ที่ port 3001
...
function add_sensors(location, sensor){
  ...
  for (k of sensor) {
    ...
    else if(sensor[count]["deviceClass"].includes('iBeacon')==true&&sensor[count]["deviceClass"].includes('eddystone')==true&&sensor[count]["deviceClass"].includes('Minew')==true&&sensor[count]['rssi']!=null){
      let sql = "";
      let param = {};
      param[0] = sensor[count]['mac'];
      param[1] = 'Minew';
